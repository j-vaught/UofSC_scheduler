    ...
    for group in elective_groups:
        ...
        if pick_credits > 0:
            ...
            if remaining_pick > 0:
                ...
                categories[group.get("category", "electives")]["remaining"] += (
                    remaining_pick * credits_each
                )

    # Compute total remaining
    remaining_required_credits = sum(c.get("credits", 3) for c in required_remaining)
    remaining_elective_credits = sum(
        g.get("remaining_credits", 0) for g in elective_groups_remaining
    )
    total_remaining = max(
        remaining_required_credits + remaining_elective_credits, total_required - completed_credits
    )
...
def plan_degree(
    ...
    warnings = _generate_warnings(
        semesters, major_map, mode, annual_credits, remaining_courses, credit_limits, offering_hints
    )
...
def _generate_warnings(
    semesters, major_map, mode, annual_credits, unplaced, credit_limits, offering_hints
):
