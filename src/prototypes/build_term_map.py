...
INPUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "data", "generated", "course_data.json")
...
def main():
    ...
    print(
        f"Vocabulary: {len(vocab)} terms (from {len(df)} total, filtered by df=[{MIN_DF}, {max_df}])"
    )
