...
**The relay worker is not a committed file.** `dist/server/index.js` is generated by
`build_static_site.py` from the `SITES_WORKER` string constant near the top of that file.
All relay behavior changes are made in that Python string. `tests/test_sites_relay_worker.js`
covers the generated output.
