/* Device-local custom major-map builder. */
(function initCustomMajorMap(root, factory) {
    const api = factory();
    if (typeof module === 'object' && module.exports) module.exports = api;
    root.CustomMajorMap = api;
    ...
    function writeAll(maps) {
        if (typeof localStorage === 'undefined') return;
        localStorage.setItem(STORAGE_KEY, JSON.stringify(maps.map(normalizeDraft)));
    }
    ...
    function selectMap(map) {
        ...
        document.getElementById('major-program-select').value = programKey;
    ...
    function updateEditButton() {
        ...
        button.hidden = !State.profile.majorData?.custom_map;
    }

    function init() {
        if (typeof document === 'undefined') return;
        document.getElementById('btn-add-custom-major-map')?.addEventListener('click', () => open());
        document.getElementById('btn-edit-custom-major-map')?.addEventListener('click', () => open(State.profile.major));
        ...
        if (typeof State !== 'undefined') State.on('profile-updated', updateEditButton);
        updateEditButton();
    }
