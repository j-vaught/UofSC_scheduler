/*
 * Offering history, fenced.
 *
 * The first feature extracted under phase 7a, chosen because it has one inbound
 * caller and two outbound dependencies -- the cleanest boundary in the tree.
 *
 * The contract is the argument list. This module names what it needs and cannot
 * reach anything else: no API global, no State global, no document lookup
 * outside its own container. Extraction surfaced a fourth dependency the old
 * code hid: _escape reached for document.createElement to escape text, which is
 * a DOM dependency in a function that reads as pure. Swapping the data source or the container is a
 * change at the call site, not in here, which is what "replaceable" has to mean.
 *
 * The body is the previous implementation verbatim apart from those three
 * seams. A fence is not a rewrite; mixing the two is how extractions introduce
 * bugs that look like refactors.
 */
(function initHistoryFeature(root, factory) {
    const api = factory();
    if (typeof module === 'object' && module.exports) module.exports = api;
    if (!root.Features) root.Features = {};
    root.Features.history = api;
}(typeof globalThis === 'object' ? globalThis : self, () => {
    'use strict';

    function createHistoryFeature(deps) {
        for (const name of ['getHistory', 'currentTerm', 'container', 'createElement']) {
            if (typeof deps?.[name] !== 'function') {
                throw new TypeError(`history feature needs a ${name}() dependency`);
            }
        }
    ...
    _historyBoundary(serverAsOfTerm = '') {
        const selected = deps.currentTerm() || this._activeTerm();
    ...
    _escape(value) {
        const node = deps.createElement('span');
        node.textContent = String(value ?? '');
        return node.innerHTML;
    },

    renderLoading(courseCode, progress = {}, container = deps.container()) {
        if (!container) return;
    ...
    async loadForCourse(courseCode) {
        ...
        try {
            const data = await deps.getHistory(courseCode, progress => {
