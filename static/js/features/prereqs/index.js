/*
 * Prerequisite lookup and major-map enrichment, fenced.
 *
 * Six dependencies, and one of them is the last remaining edge out of this
 * module into Search: showCourseDetail, which is navigation rather than data.
 ...
 */
(function initPrereqsFeature(root, factory) {
    const api = factory();
    if (typeof module === 'object' && module.exports) module.exports = api;
    if (!root.Features) root.Features = {};
    root.Features.prereqs = api;
}(typeof globalThis === 'object' ? globalThis : self, () => {
    'use strict';

    function createPrereqsFeature(deps) {
        for (const name of ['bulletinSearch', 'bulletinDetails', 'searchCourses',
            'completedCourses', 'currentTerm', 'showCourseDetail']) {
            if (typeof deps?.[name] !== 'function') {
                throw new TypeError(`prereqs feature needs a ${name}() dependency`);
            }
        }

        const feature = {        _cache: {},
        _loadId: 0,
        ...
        async loadForCourse(courseCode) {
            ...
            try {
                ...
                const search = await deps.bulletinSearch(subject);
                ...
                const details = await deps.bulletinDetails(target.key);
                if (loadId !== this._loadId) return;
                ...
                const completed = new Set(deps.completedCourses());
        ...
        async navigateToCourse(courseCode) {
            ...
            try {
                const [live, bulletin] = await Promise.all([
                    deps.searchCourses(deps.currentTerm(), [{ field: 'subject', value: subject }]),
                    deps.bulletinSearch(subject),
                ]);
        ...
        async loadDegreeRequirements(course, catalogYear, catalogByCode = new Map()) {
            ...
            if (catalog.key) {
                try {
                    details = await deps.bulletinDetails(catalog.key, catalogYear);
                } catch (error) {
                    details = catalog;
                }
            }
        ...
        async enrichMajorMap(majorMap) {
            ...
            await Promise.all(subjects.map(async subject => {
                try {
                    const response = await deps.bulletinSearch(subject, catalogYear);
