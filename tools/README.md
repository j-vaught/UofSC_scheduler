...
```bash
uv run python tools/<name>.py --help
```

> The directory name is load-bearing. `from tools.X import Y` appears in eight
> modules and in the tests, resolved through `tools/__init__.py`. Renaming the
...
Stages 1-10 are independent and can be re-run in isolation. Stage 11 consumes their
output, and stage 12 packages it.

The `src/` entry points below live under `tools/src/`; the table shows them relative
to this directory.
...
| 9 | `normalize_major_maps.py` | `data/curated/major_maps` | `--report` |
| 10 | `validate_major_maps.py` | maps dir + active manifest | `--json-output` |
| 11 | `build_static_release.py` | everything above | `static/data/releases/<id>/` + `manifest.json` |
| 12 | `build_static_site.py` | `static/` | `dist/client/` + `dist/server/` |
...
**Stage 11 is one in-process step.** `build_static_release.py` imports the shard builders
...
**`--history-cache` and `--term-dir` are mutually exclusive, and one is required.** Stage 11
needs section history from exactly one of them.

**`data/grade_matching_cache.sqlite` is gitignored but is the default stage-11 input.** A
...
**External binaries.** `import_major_maps.py` needs `pdftotext`; `download_major_map_pdfs.py` needs `pdfinfo`. Both come from Poppler.
...
Nothing reaches a release unvalidated. `validate_major_maps.py` cross-checks course codes
against the active catalog. `build_static_site.py` independently re-verifies every manifest artifact's
